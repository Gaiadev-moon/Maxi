...
function CashSummaryCard({ cashSession, sales, businessSales }: { cashSession?: CashSession; sales: Sale[]; businessSales: Sale[] }) {
  ...
  return <div className={styles.cashSummaryCard}><div><span>Caja unica</span><strong>Abierta</strong><small>Desde {date(cashSession.openedAt)}</small></div><div className={styles.cashSummaryRows}><Total label="Efectivo inicial" value={cashSession.openingAmount} /><Total label="Vendido en jornada" value={salesTotal(businessSales)} /><Total label="Total desde apertura" value={salesTotal(sales)} /><Total label="Efectivo esperado" value={cashExpected(cashSession, sales)} /></div></div>;
}
